...
function addWordToSettingsAndUpdate(filename, word) {
    return readApplyUpdateSettingsFile(filename, settings => addWordsToSettings(settings, word.split(' ')));
}
...
function addIgnoreWordToSettingsAndUpdate(filename, word) {
    return readApplyUpdateSettingsFile(filename, settings => addIgnoreWordsToSettings(settings, word.split(' ')));
}
exports.addIgnoreWordToSettingsAndUpdate = addIgnoreWordToSettingsAndUpdate;
function addIgnoreWordsToSettings(settings, wordsToAdd) {
    const ignoreWords = mergeWords(settings.ignoreWords, wordsToAdd);
    return Object.assign({}, settings, { ignoreWords });
}
exports.addIgnoreWordsToSettings = addIgnoreWordsToSettings;
function mergeWords(wordsLeft, wordsRight) {
    return (wordsLeft || [])
        .concat(wordsRight)
        .map(a => a.trim())
        .filter(a => !!a)
...
function removeWordFromSettingsAndUpdate(filename, word) {
    return readApplyUpdateSettingsFile(filename, settings => removeWordsFromSettings(settings, word.split(' ')));
}
...
function writeAddLanguageIdsToSettings(filename, languageIds, onlyIfExits) {
    return readApplyUpdateSettingsFile(filename, settings => addLanguageIdsToSettings(settings, languageIds, onlyIfExits));
}
...
function removeLanguageIdsFromSettingsAndUpdate(filename, languageIds) {
    return readApplyUpdateSettingsFile(filename, settings => removeLanguageIdsFromSettings(settings, languageIds));
}
exports.removeLanguageIdsFromSettingsAndUpdate = removeLanguageIdsFromSettingsAndUpdate;
async function readApplyUpdateSettingsFile(filename, action) {

...
class App extends PureComponent {
	...
	handleAddFavorite = event => {
		...
		if (this.state.favorites.has(name)) {
			this.setState({ error: {  show: true, message: DuplicateFavorite, level: WARN });
		}

		this.setState(prevState => {
			return {
				favorites: prevState.favorites.set(name,{ owner, name, rawurl, lang, content })
		...
		client
			...
			.catch(err => {
				if (err.response && err.response.data.message === 'Not Found') {
					this.setState({ error: { show:true, message: NoUserErrorMsg, level:ERROR} });
				}
			})

...
class App extends PureComponent {
	...
	handleAddFavorite = event => {
		const { owner, filename, rawurl, content, lang } = event.target.dataset;
		this.setState(prevState => {
			return {
				files: [...prevState.files, { owner, filename, rawurl, content }]
			};
		});
	};
	...
	render() {
		...
		return (
			<div className='container-large' style={styles.body}>
				...
				{favorites.length ? <Favorite files={favorites} /> : null}
				{gists ? (
					<Provider value={this}>

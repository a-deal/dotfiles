...
class App extends PureComponent {
	...
	handleAddFavorite = event => {
		...
		if (this.state.favorites.has(name)) {
			this.setState({
				error: { show: true, message: DuplicateFavorite, level: WARN }
			});
		}
		...
		client
			...
			.catch(err => {
				if (err.response && err.response.data.message === 'Not Found') {
					this.setState({
						error: { show: true, message: NoUserErrorMsg, level: ERROR }
					});
				}
			})
	...
	handleGistResults = gists => {
		if (gists.length) {
			...
			this.setState({
				error: { show: true, message: NoGistsErrorMsg, level: WARN }
			});
		}
	};
	render() {

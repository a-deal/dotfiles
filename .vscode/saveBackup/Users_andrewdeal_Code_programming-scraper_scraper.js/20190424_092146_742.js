...
async function wodScraper(url, browser) {
  ...
  $('main > section > div')
    ...
    .each((i, e) => {
      let $e = $(e);
      let isSectionHeader = $e.has('strong').length;

      if (isSectionHeader && section.length) {
        WOD[sectionHeader] = content;
        (section = $e.find('strong').text()), (content = '');
      } else if (isSectionHeader) {
      }
      if ($e.has('strong')) section = $e.find('strong').text();
